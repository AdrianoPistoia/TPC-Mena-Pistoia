    ...
    <asp:Panel runat="server">
        <asp:Panel ID="containerResultado" runat="server" Style="border: 5px black solid;" CssClass="BorderShadowLB  DisInline VerticalStretch1200px">
            ...
            <div class="container BGCutePink">
                <asp:Panel runat="server" CssClass="BGWarmGreen2" Style="border-bottom: 5px solid black;border-left: 5px solid black;border-right: 5px solid black; width: 83.5vw; height: 1170px; display:flex;">
                    <asp:Panel ID="CartaProducto" style="background-color: rgba(19,14,10,0.3);height:max-content;width:1200px;flex-wrap:wrap;justify-content:space-between" CssClass="CartaProducto " runat="server">
                    <!-- lista del carrito -->
                    <% 
                        foreach (Dominio.Producto Producto in listaFavoritos)
                        { %>
                        <div id="prod" class="BGLightBeige" style="width: 24rem;height:36rem; border: solid 3px black; padding: 10px; margin: 5px;display:flex;flex-direction:column;align-items:center">
                            ...
                            <a type="button" style="background-color:rgba(206,60,50,0.9)" class="btn Bolder btn-secondary" onclick="<%  %>" href="/Tienda.aspx?id=<%: Producto.ID.ToString() %>" >Agregar al carrito!</a>
                        </div>
                        <!-- fin de  producto-->    
                        <%}%>
                    </asp:Panel>
